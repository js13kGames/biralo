...
export class Quadtree {
	...
	// constructor({ maxDepth = 3, maxObjects = 25, canvas, bounds } = {}) {
	/**
	 * @param {object} [props] - Properties of the quadtree.
	 * @param {number} [props.maxDepth] - Maximum node depth of the quadtree.
	 * @param {number} [props.maxObjects] - Maximum number of objects a node can have before splitting.
	 * @param {{x: number, y: number, width: number, height: number}} [props.bounds] - The 2D space (x, y, width, height) the quadtree occupies. Defaults to the entire canvas width and height.
	 * @param {{width: number, height: number}} props.canvas - Game canvas dimension.
	 */
	constructor(props) {
		 ...
		 */
		this.maxDepth = props.maxDepth ?? 3;
		 ...
		 */
		this.maxObjects = props.maxObjects ?? 25;
