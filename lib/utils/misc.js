...
export function hitTest(r1, r2) {
	...
	if (Math.abs(dx) < dw && Math.abs(dy) < dh) {
		...
		if (collision.overlap.x >= collision.overlap.y) {
			collision.side = dy > 0 ? 't' : 'b';
		}
		else {
			collision.side = dx > 0 ? 'l' : 'r';
		}
	}
 ...
 * @type {FOverlap}
 */
export function overlap(r1, r2, dw=1, dh=1) {
	var isInHoriztonalBounds = r1.x < r2.x + r2.width*dw && r1.x + r1.width*dw > r2.x;
	var isInVerticalBounds = r1.y < r2.y + r2.height*dh && r1.y + r1.height*dh > r2.y;
